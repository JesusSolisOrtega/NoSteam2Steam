...
check_dependencies() {
    ...
    if [ ${#missing_deps[@]} -ne 0 ]; then
        echo "Missing dependencias: ${missing_deps[*]}"
        ...
        if [ ${#failed_installs[@]} -ne 0 ]; then
            echo "Error: not every dependency could be installed."
            echo "Please, install: ${failed_installs[*]}"
            echo "You can use: sudo pacman -S ${failed_installs[*]}"
            exit 1
        fi
    fi
}
